...
function testcontainers_get_dependencies() {
  if [ "$testcontainers_enabled" = "true" ]; then
    if [ "$testcontainers_rootless" = "true" ]; then
      ...
      # pasta, podman's modern rootless network backend (replaces slirp4netns).
      echo "ca-certificates curl gawk iproute2 jq podman podman-docker fuse-overlayfs passt uidmap passwd"
    else
